...
def test_config_defaults_match_plan() -> None:
    ...
    assert config.connection_confidence_threshold == 0.10
    assert config.temporal_window_days == 90
    assert config.max_agent_searches == 5
    assert config.max_connection_candidates_per_ticker == 12
    assert config.max_valid_connections_per_ticker == 4
